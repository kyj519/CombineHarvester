...
int main()
{
  ...
  //! [part3]
  cb.AddObservations({"*"}, {"WtoCB"}, {"2016preVFP"}, {"BvsC_3rd_4th_Jets_Unrolled"}, cats);
  //! [part3]
  ...
  cb.AddProcesses({"*"}, {"WtoCB"}, {"2016preVFP"}, {"BvsC_3rd_4th_Jets_Unrolled"}, bkg_procs, cats, false);

  // cb.AddProcesses({"*"}, {"WtoCB"}, {"2016preVFP"}, {"BvsC_3rd_4th_Jets_Unrolled"}, bkg_procs_non_ttbar, cats, false);

  vector<string> sig_procs = {"WtoCB"};
  cb.AddProcesses({"*"}, {"WtoCB"}, {"2016preVFP"}, {"Template_MVA_Score"}, sig_procs, cats, true);
  //! [part4]
  ...
  //! [part5]
  cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "lumi_uncorr_$ERA", "lnN", SystMap<era>::init({"2016preVFP"}, 1.020));
    cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "lumi_corr_16_17_18", "lnN", SystMap<era>::init({"2016preVFP"}, 1.006));
  ...
  cb.cp().process({{"ttV"}}).AddSyst(cb, "ttVNorm", "lnN", SystMap<era>::init({"2016preVFP"}, 1.2));
  cb.cp().process({{"VV"}}).AddSyst(cb, "VVNorm", "lnN", SystMap<era>::init({"2016preVFP"}, 1.2));
  cb.cp().process({{"ST"}}).AddSyst(cb, "STNorm", "lnN", SystMap<era>::init({"2016preVFP"}, 1.2));
  cb.cp().process({{"VJets"}}).AddSyst(cb, "VJetsNorm", "lnN", SystMap<era>::init({"2016preVFP"}, 1.2));
  cb.cp().process({{"QCD_bEn"}}).AddSyst(cb, "QCDNorm", "lnN", SystMap<era>::init({"2016preVFP"}, 2.0));

  // cb.cp().process(ttbar_2).AddSyst(cb, "udNorm", "lnN", SystMap<era>::init({"2016preVFP"}, 1.2));
  // cb.cp().process(ttbar_4).AddSyst(cb, "csNorm", "lnN", SystMap<era>::init({"2016preVFP"}, 1.2));
  ...
  cb.cp().backgrounds().ExtractShapes(
      aux_shapes + "/Vcb_DL_Histos_2016preVFP_EE_processed.root",
  ...
  TFile output("Vcb_Template_EE_2016preVFP.root", "RECREATE");
