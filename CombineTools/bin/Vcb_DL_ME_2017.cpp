...
int main()
{
  ...
  //! [part3]
  cb.AddObservations({"*"}, {"WtoCB"}, {"2017"}, {"BvsC_3rd_4th_Jets_Unrolled"}, cats);
  //! [part3]
  ...
  cb.AddProcesses({"*"}, {"WtoCB"}, {"2017"}, {"BvsC_3rd_4th_Jets_Unrolled"}, bkg_procs, cats, false);

  // cb.AddProcesses({"*"}, {"WtoCB"}, {"2017"}, {"BvsC_3rd_4th_Jets_Unrolled"}, bkg_procs_non_ttbar, cats, false);

  vector<string> sig_procs = {"WtoCB"};
  cb.AddProcesses({"*"}, {"WtoCB"}, {"2017"}, {"Template_MVA_Score"}, sig_procs, cats, true);
  //! [part4]
  ...
  //! [part5]
  cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "lumi_uncorr_$ERA", "lnN", SystMap<era>::init({"2017"}, 1.020));
      cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "lumi_corr_17_18", "lnN", SystMap<era>::init({"2017"}, 1.006));
    cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "lumi_corr_16_17_18", "lnN", SystMap<era>::init({"2017"}, 1.009));
  ...
  cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "Mu_Trig_", "shape", SystMap<>::init(1.00));
  // cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "El_Trig_", "shape", SystMap<>::init(1.00));
  ...
  cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "El_Reco_", "shape", SystMap<>::init(1.00));
  ...
  // cb.cp().process(ch::JoinStr({sig_procs, bkg_procs})).AddSyst(cb, "PDF_Error_Set_", "shape", SystMap<>::init(1.00));
  ...
  cb.cp().process(ch::JoinStr({sig_procs, bkg_proc_ttbar})).AddSyst(cb, "hDamp_", "shape", SystMap<>::init(1.00));
  cb.cp().process(ch::JoinStr({sig_procs, bkg_proc_ttbar})).AddSyst(cb, "mTop_", "shape", SystMap<>::init(1.00));
  cb.cp().process(ch::JoinStr({sig_procs, bkg_proc_ttbar})).AddSyst(cb, "Top_Pt_Reweight_", "shape", SystMap<>::init(1.00));
  cb.cp().process(ch::JoinStr({sig_procs, bkg_proc_ttbar})).AddSyst(cb, "CP5_", "shape", SystMap<>::init(1.00));

  cb.cp().process({{"ttV"}}).AddSyst(cb, "ttVNorm", "lnN", SystMap<era>::init({"2017"}, 1.2));
  cb.cp().process({{"VV"}}).AddSyst(cb, "VVNorm", "lnN", SystMap<era>::init({"2017"}, 1.2));
  cb.cp().process({{"ST"}}).AddSyst(cb, "STNorm", "lnN", SystMap<era>::init({"2017"}, 1.2));
  cb.cp().process({{"VJets"}}).AddSyst(cb, "VJetsNorm", "lnN", SystMap<era>::init({"2017"}, 1.2));
  cb.cp().process({{"QCD_bEn"}}).AddSyst(cb, "QCDNorm", "lnN", SystMap<era>::init({"2017"}, 2.0));

  // cb.cp().process(ttbar_2).AddSyst(cb, "udNorm", "lnN", SystMap<era>::init({"2017"}, 1.2));
  // cb.cp().process(ttbar_4).AddSyst(cb, "csNorm", "lnN", SystMap<era>::init({"2017"}, 1.2));
  // cb.cp().bin({"MM"}).AddSyst(cb,"ttbbxsec","rateParam",SystMap<process>::init({"BB_TTLJ","BB_TTLL"}, 1.36));
  // cb.cp().bin({"MM"}).AddSyst(cb,"ttccxsec","rateParam",SystMap<process>::init({"CC_TTLJ","CC_TTLL"}, 1.11));
  // cb.cp().bin({"MM"}).AddSyst(cb,"ttjjxsec","rateParam",SystMapFunc<process>::init({"JJ_TTLJ","JJ_TTLL"},"(453.63-@0*10.42-@1*34.82)/(408.39)", "ttbbXsec,ttccXsec"));
  ...
  cb.cp().backgrounds().ExtractShapes(
      aux_shapes + "/Vcb_DL_Histos_2017_ME_processed.root",
      "Nominal/$PROCESS/BvsC_3rd_4th_Jets_Unrolled",
      "$SYSTEMATIC/$PROCESS/BvsC_3rd_4th_Jets_Unrolled");
  ...
  TFile output("Vcb_Template_ME_2017.root", "RECREATE");
  ...
  for (auto b : bins)
  {
    ...
    cb.cp().bin({b}).WriteDatacard(b + ".txt", output);
