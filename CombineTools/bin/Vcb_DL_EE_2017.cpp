...
int main()
{
  ...
  // cb.cp().bin({"EE"}).AddSyst(cb,"ttbbxsec","rateParam",SystMap<process>::init({"BB_TTLJ","BB_TTLL"}, 1.36));
  // cb.cp().bin({"EE"}).AddSyst(cb,"ttccxsec","rateParam",SystMap<process>::init({"CC_TTLJ","CC_TTLL"}, 1.11));
  // cb.cp().bin({"EE"}).AddSyst(cb,"ttjjxsec","rateParam",SystMapFunc<process>::init({"JJ_TTLJ","JJ_TTLL"},"(453.63-@0*10.42-@1*34.82)/(408.39)", "ttbbXsec,ttccXsec"));
  ...
  cb.cp().backgrounds().ExtractShapes(
      aux_shapes + "/Vcb_DL_Histos_2017_EE_processed.root",
  ...
  TFile output("Vcb_Template_EE_2017.root", "RECREATE");
