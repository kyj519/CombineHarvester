...
int main()
{
    ...
    ch::Categories cats = {{1, "Signal"},{2, "Control0"}};
    vector<string> channels = {"Mu", "El"};
    ...
    cb.cp().AddSyst(cb, "Mu_Trig_", "shape", SystMap<channel>::init({"Mu"},1.00));
    cb.cp().AddSyst(cb, "El_Trig_", "shape", SystMap<channel>::init({"El"},1.00));
    
    //Muon ID and Res: https://twiki.cern.ch/twiki/bin/viewauth/CMS/TopSystematics#Muon_identification_isolation_mo
    cb.cp().AddSyst(cb, "Mu_Id_", "shape", SystMap<channel>::init({"Mu"},1.00));
    cb.cp().AddSyst(cb, "Mu_Iso_", "shape", SystMap<channel>::init({"Mu"},1.00));

    //El ID and Reco: https://twiki.cern.ch/twiki/bin/viewauth/CMS/TopSystematics#Electron_and_photon_identificati
    cb.cp().AddSyst(cb, "El_Id_", "shape", SystMap<channel>::init({"El"},1.00));
    cb.cp().AddSyst(cb, "El_Reco_", "shape", SystMap<channel>::init({"El"},1.00));
