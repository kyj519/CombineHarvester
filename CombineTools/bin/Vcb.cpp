...
int WriteFinalLine(string fname, vector<string> *procs)
{
    ...
    if (datacardFile.is_open())
    {
        ...
        datacardFile << size_t pos = fname.rfind("_");

        // Find the second last occurrence of "_"
        size_t second_last_pos = fname.rfind("_", pos - 1);

        // Extract the substring between the second last and last occurrence of "_"
        std::string era = fname.substr(second_last_pos + 1, pos - second_last_pos - 1);

        if (era == "2016preVFP" or era == "2016postVFP")
        {
            // deccorrelate LFStats and HFStats bewteen two 2016's
            datacardFile << "nuisance edit rename * * B_Tag_LFStats1_2016_ B_Tag_LFStats1_" << era << "_\n";
            datacardFile << "nuisance edit rename * * B_Tag_LFStats2_2016_ B_Tag_LFStats2_" << era << "_\n";
            datacardFile << "nuisance edit rename * * B_Tag_HFStats1_2016_ B_Tag_HFStats1_" << era << "_\n";
            datacardFile << "nuisance edit rename * * B_Tag_HFStats2_2016_ B_Tag_HFStats2_" << era << "_\n";
        }
        /*
                ...
                if (fname.find("2016") != std::string::npos)
                {
                    ...
                    if (fname.find("2016preVFP") != std::string::npos)
                    {
                        for (auto syst : decorr_systs)
                        {
                            for (auto proc : *procs)
                            {
                                // if channel, syst is (EE,Mu_Trig), (El,Mu_Trig), (MM,El_Trig), (Mu,El_Trig), skip

                                if ((fname.find("EE") != std::string::npos && syst.find("Mu_Trig") != std::string::npos) || (fname.find("El") != std::string::npos && syst.find("Mu_Trig") != std::string::npos) || (fname.find("MM") != std::string::npos && syst.find("El_Trig") != std::string::npos) || (fname.find("Mu") != std::string::npos && syst.find("El_Trig") != std::string::npos))
                                {
                                    continue;
                                }
...
int main()
{
    ...
    cb.cp().process(ch::JoinStr({bkg_proc_ttbar, sig_procs})).AddSyst(cb, "ttxsec", "lnN", SystMap<>::init(1.0439));

    // Luminosity:https://twiki.cern.ch/twiki/bin/view/CMS/LumiRecommendationsRun2#
    cb.cp().AddSyst(cb, "lumi_uncorr_$ERA_", "lnN", SystMap<era>::init({"2018"}, 1.015)({"2017"}, 1.020));
    cb.cp().AddSyst(cb, "lumi_uncorr_2016_", "lnN", SystMap<era>::init({"2016preVFP"}, 1.01)({"2016postVFP"}, 1.01));
    ...
    // L1 Prefire
    cb.cp().AddSyst(cb, "Prefire_$ERA_", "shape", SystMap<era>::init({"2018", "2017"}, 1.0));
    cb.cp().AddSyst(cb, "Prefire_2016_", "shape", SystMap<era>::init({"2016preVFP", "2016postVFP"}, 1.0));
    ...
    cb.cp().AddSyst(cb, "Mu_Trig_$ERA_", "shape", SystMap<channel, era>::init({"MM", "ME"}, {"2017", "2018"}, 1.00));
    cb.cp().AddSyst(cb, "Mu_Trig_2016_", "shape", SystMap<channel, era>::init({"MM", "ME"}, {"2016preVFP", "2016postVFP"}, 1.00));

    cb.cp().AddSyst(cb, "El_Trig_$ERA_", "shape", SystMap<channel, era>::init({"EE", "ME"}, {"2017", "2018"}, 1.00));
    cb.cp().AddSyst(cb, "El_Trig_2016_", "shape", SystMap<channel, era>::init({"EE", "ME"}, {"2016preVFP", "2016postVFP"}, 1.00));
    ...
    // PU ID SF
    cb.cp().AddSyst(cb, "PU_Jet_Veto_$ERA_", "shape", SystMap<era>::init({"2018", "2017"}, 1.00));
    cb.cp().AddSyst(cb, "PU_Jet_Veto_2016_", "shape", SystMap<era>::init({"2016preVFP", "2016postVFP"}, 1.00));
    ...
    cb.cp().AddSyst(cb, "Jet_Res_$ERA_", "shape", SystMap<era>::init({"2018", "2017"}, 1.00));
    cb.cp().AddSyst(cb, "Jet_Res_2016_", "shape", SystMap<era>::init({"2016preVFP", "2016postVFP"}, 1.00));

    // B-Tagging: https://twiki.cern.ch/twiki/bin/viewauth/CMS/BTagShapeCalibration#Correlation_across_years_2016_20
    cb.cp().AddSyst(cb, "B_Tag_LFStats1_$ERA_", "shape", SystMap<era>::init({"2018", "2017"}, 1.00));
    cb.cp().AddSyst(cb, "B_Tag_LFStats1_2016_", "shape", SystMap<era>::init({"2016preVFP", "2016postVFP"}, 1.00));

    cb.cp().AddSyst(cb, "B_Tag_LFStats2_$ERA_", "shape", SystMap<era>::init({"2018", "2017"}, 1.00));
    cb.cp().AddSyst(cb, "B_Tag_LFStats2_2016_", "shape", SystMap<era>::init({"2016preVFP", "2016postVFP"}, 1.00));

    cb.cp().AddSyst(cb, "B_Tag_HFStats1_$ERA_", "shape", SystMap<era>::init({"2018", "2017"}, 1.00));
    cb.cp().AddSyst(cb, "B_Tag_HFStats1_2016_", "shape", SystMap<era>::init({"2016preVFP", "2016postVFP"}, 1.00));

    cb.cp().AddSyst(cb, "B_Tag_HFStats2_$ERA_", "shape", SystMap<era>::init({"2018", "2017"}, 1.00));
    cb.cp().AddSyst(cb, "B_Tag_HFStats2_2016_", "shape", SystMap<era>::init({"2016preVFP", "2016postVFP"}, 1.00));
    ...
    cb.cp().AddSyst(cb, "UE_$ERA_", "shape", SystMap<era>::init({"2018", "2017"}, 1.00));
    cb.cp().AddSyst(cb, "UE_2016_", "shape", SystMap<era>::init({"2016preVFP", "2016postVFP"}, 1.00));
